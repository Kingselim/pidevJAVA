...
import com.esprit.controllers.ActionController.ShowDetailsAction;
...
public class MenuClient implements Initializable {
    ...
    public void setData(String dataToPass) {
        ...
        idUserpourlarecherche = user.getId();
        ...
        try {
            ...
            if(res<7 && wallet.getNbrconnection()<3)
            {
                wallet.setRate(1);
            ...
            if(res<7 && wallet.getNbrconnection()>=3 && wallet.getNbrconnection()<5)
            {
                wallet.setRate(2);
                File file = new File("images/rate2.png");
                Image myImage = new Image(String.valueOf(file));
                imagerate.setImage(myImage);
            }
            if(res<7 && wallet.getNbrconnection()>=5 && wallet.getNbrconnection()<8)
            {
                wallet.setRate(3);
            ...
            if(res<7 && wallet.getNbrconnection()>=8 && wallet.getNbrconnection()<10)
            {
                wallet.setRate(4);
                File file = new File("images/rate4.png");
                Image myImage = new Image(String.valueOf(file));
                imagerate.setImage(myImage);
            }
    ...
    public void setData2(String dataToPass) {
        //label.setText("L'utilisateur avec l'ID : "+dataToPass);
        //System.out.println("la valeur datatopass st"+dataToPass);
        ...
        int newNbrConnection=oldNbrConnection+0; //+ 0 le nbr de conn
        System.out.println("le nombre de connection selevre a :"+newNbrConnection);
        wallet= new Wallet(wallet.getId(),wallet.getRate(), String.valueOf(LocalDate.now()),newNbrConnection, wallet.getIdofuser_id());
        System.out.println("dans le wallet cree nbconn ==="+wallet.getNbrconnection());
        walletService.modifier(wallet);
    ...
    void goassociation(ActionEvent event) throws IOException{
        ...
        controller.SetUserId(idUserpourlarecherche);
