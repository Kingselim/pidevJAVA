...
public class Seconnecter implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle)
    {
       ...
       // System.out.println(chatGPTAPIExample.chatGPT("hello, how are you? Can you tell me what's a Fibonacci Number?"));
    ...
    void seconnecter(ActionEvent event) {
        ...
        {
            User user = new User();
            user = service.recupererUser(email.getText());
            // Affichage d'un SMS de confirmation de creation de compte
            String codeDoubleAuth = generateRandomString(4);
            String SmsToSend ="Double Authentification voici le code a saisir :"+codeDoubleAuth+"!";
            // decommenter pour faire fonctionner SMS
            //TwilioSMSUser.sendCustomMessage("+21650111079",SmsToSend);

            // decommenter pour faire fonctionner Email
            service.envoyermail(user,codeDoubleAuth);
            ...
            // Load and show the new interface
            try {
                FXMLLoader loader = new FXMLLoader(getClass().getResource("/DoubleAuthuser.fxml"));
                Parent root = loader.load();
                DoubleAuthuser controller = loader.getController();
                controller.setData(codeDoubleAuth);

                Stage newStage = new Stage();
                newStage.setScene(new Scene(root));
                newStage.show();
                newStage.setTitle("Double Authentification");
            } catch (IOException e) {
                e.printStackTrace();
                // Handle exception, if any
            }
